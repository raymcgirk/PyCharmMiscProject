...
def copy_file(file_path: Path, base: Path, destinations: list[Path], existing_index: set[tuple[str, int]]):
    rel_path = relative_path(file_path, base)
    size = file_path.stat().st_size

    if already_copied(str(rel_path), size, existing_index):
...
def main():
    ...
    with ThreadPoolExecutor(max_workers=MAX_WORKERS) as executor:
        futures = [
            executor.submit(copy_file, file_path, base, DESTINATIONS, existing_index)
